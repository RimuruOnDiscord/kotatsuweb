  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={`flex h-11 select-none items-center gap-2.5 rounded-[16px] border px-4 text-[13px] font-bold transition-all duration-500 outline-none backdrop-blur-md
          ${isOpen
      ...
      {isOpen && (
        <div className="absolute left-0 top-[calc(100%+10px)] z-50 min-w-[220px] max-h-[380px] overflow-y-auto rounded-[20px] border border-white/10 bg-[color-mix(in_srgb,var(--app-accent),transparent_95%)] p-2 backdrop-blur-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.6)] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex flex-col gap-1">
          {options.map((opt) => {
  ...
  return (
    <div className="hidden xl:flex flex-wrap items-center gap-2.5">
      ...
      <div className="group relative flex min-w-[240px] max-w-[300px] flex-1 items-center">
        ...
        <X 
          size={14} 
          className="transition-colors duration-300 hover:text-[var(--app-accent)]" 
        />
          </button>
        )}
