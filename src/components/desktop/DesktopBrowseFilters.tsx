...
const FilterSelect: React.FC<{
  ...
  const OptionButton: React.FC<{
    ...
    <button
      ...
      onClick={onClick}
      onMouseDown={option.disabled ? undefined : handleRippleMouseDown}
    ...
    >
      <span className={`text-[12px] font-medium transition-colors duration-200 ${option.disabled ? 'text-zinc-600' : isSelected ? 'text-emerald-300' : ''}`}>
        {option.label}
      </span>
      <Check
        size={12}
  ...
  const panelClass = `mt-3 origin-top overflow-hidden rounded-xl border border-white/[0.06] bg-[#1a1b1e] will-change-transform transition-[transform,opacity,box-shadow,filter] ${
    isOpen
    ...
    <div className="flex w-full flex-wrap items-center gap-2 overflow-visible pb-2 xl:flex-nowrap">
      ...
      {hasActiveFilters ? (
        <button
          ...
          onMouseDown={handleRippleMouseDown}
          className="ripple-button inline-flex h-11 items-center justify-center rounded-2xl border border-white/[0.06] bg-[#15171a] px-4 text-[10px] font-black uppercase tracking-[0.18em] text-zinc-300 transition-colors hover:border-white/[0.1] hover:bg-[#181a1d] hover:text-white"
        >
          Clear
        </button>
      ) : null}
