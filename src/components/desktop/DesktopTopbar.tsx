  ...
  return (
    <div className="mx-auto hidden w-full max-w-[1420px] items-center justify-between gap-6 px-6 py-4 lg:flex">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <button onClick={handleToggleMode} className="flex h-10 items-center gap-2 rounded-full border border-[var(--app-border)] bg-[var(--app-surface-1)] px-4 text-[13px] font-semibold text-white transition-all hover:bg-[var(--app-surface-2)] active:scale-95">
          {isAnimeMode ? <Tv size={15} className="text-white-400" /> : <Book size={15} className="text-white-400" />}
        </button>
        <button onClick={() => navigate('/settings')} className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--app-border)] bg-[var(--app-surface-1)] text-zinc-400 transition-all hover:bg-[var(--app-surface-2)] hover:text-white">
          <Settings size={18} />
        </button>
      </div>
    </div>
