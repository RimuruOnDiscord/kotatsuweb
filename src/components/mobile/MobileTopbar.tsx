...
import { Menu, Search, X, Settings as SettingsIcon, User, Home, Globe, Bookmark, Calendar, Dices } from 'lucide-react';
import { NavLink, useNavigate, useLocation } from 'react-router-dom';
import { BrandLogo, SearchResult, TopbarSearchResultsContent } from '../shared/topbarShared';
import { getStoredTheme, setTheme, ThemeKey } from '../../utils/theme';
...
const mobileNavItems = [
  ...
  { to: '/browse', label: 'Discover', icon: Globe },
  { to: '/bookmarks', label: 'My List', icon: Bookmark },
...
const MobileTopbar: React.FC<MobileTopbarProps> = ({
  ...
  // We keep isAnimeMode to pass to the search results so they format correctly for TV mode
  ...
  const handleNavigate = (slug?: string) => {
    ...
    if (slug && typeof slug === 'string') {
      // Always use watch path since we are keeping it on TV mode
      navigate(`/watch/${slug}`);
    } else {
      // If user submits the search term
