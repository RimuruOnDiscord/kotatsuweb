...
const MobileTopbar: React.FC<MobileTopbarProps> = ({
  ...
  const handleNavigate = (result?: SearchResult) => {
    ...
    if (result) {
      ...
      const path = isAnimeMode ? `/watch/${id}` : `/read/${id}`;
      navigate(path);
    } else {
    ...
    <div className="relative lg:hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => { setSearchOpen(!searchOpen); setMenuOpen(false); }} className="h-11 w-11 flex items-center justify-center rounded-full border border-[var(--app-border)] bg-[var(--app-surface-1)] text-zinc-300 transition-all hover:text-white">
            {searchOpen ? <X size={16} /> : <Search size={16} />}
          </button>
          <button onClick={() => { setMenuOpen(!menuOpen); setSearchOpen(false); }} className="h-11 w-11 flex items-center justify-center rounded-full border border-[var(--app-border)] bg-[var(--app-surface-1)] text-zinc-300 transition-all hover:text-white">
