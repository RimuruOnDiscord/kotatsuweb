...
const handleCommentNavigation = (navigate: any, pageType: string, pageId: string, onClose: () => void) => {
  onClose();

  // 1. If pageId is already a full path, navigate directly
  if (pageId.includes('/')) {
    navigate(pageId.startsWith('/') ? pageId : `/${pageId}`);
    return;
  }

  // 2. Intercept "anime-146493-ep-1" and convert to "/watch/146493/kiwi/sub/animepahe-1"
  const animeMatch = pageId.match(/anime-(\d+)-ep-(\d+)/);
  if (animeMatch) {
    const malId = animeMatch[1];
    const episodeNum = animeMatch[2];
    navigate(`/watch/${malId}/kiwi/sub/animepahe-${episodeNum}`);
    return;
  }

  // 3. Intercept manga format "manga-123-ch-5" (Fallback)
  const mangaMatch = pageId.match(/manga-(\d+)-ch-([\d.]+)/);
  if (mangaMatch) {
    navigate(`/read/${mangaMatch[1]}?chapter=${mangaMatch[2]}`);
    return;
  }

  // 4. Absolute Fallback if there is no episode/chapter number
  const genericMatch = pageId.match(/(?:anime|manga)-(\d+)/);
  if (genericMatch) {
    const isAnime = pageType === 'anime' || pageId.includes('anime');
    navigate(`/${isAnime ? 'watch' : 'read'}/${genericMatch[1]}`);
    return;
  }
                            ...
                            {(comments.length === 0 && bookmarks.length === 0) ? (
                              ...
                              <div className="space-y-3">
                                {comments.slice(0, 3).map(c => (
                                  <div
                                    key={c.id}
                                    onClick={() => handleCommentNavigation(navigate, c.page_type, c.page_id, onClose)}
                        ...
                        <motion.div key="replies" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
                          {replies.length === 0 ? (
                            ...
                            replies.map(r => (
                              <div
                                key={r.id}
                                onClick={() => handleCommentNavigation(navigate, r.page_type, r.page_id, onClose)}
                                className="p-4 rounded-[16px] border border-white/[0.04] bg-white/[0.01] hover:bg-white/[0.02] cursor-pointer transition-all"
                              >
