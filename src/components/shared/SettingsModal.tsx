...
import {
  Palette, X, User, Loader2, LogOut, Upload, Camera,
                            ...
                            <div className="flex items-center gap-5 p-5 rounded-[16px]" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}>
                              <div className="relative group flex-shrink-0 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                                <div className="w-[72px] h-[72px] rounded-full overflow-hidden flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
                                  {avatarPreview ? <img src={avatarPreview} alt="Avatar" className="w-full h-full object-cover" /> : <User size={28} className="text-zinc-600" strokeWidth={1} />}
                                </div>
                              ...
                              <div className="flex flex-col gap-2 flex-1 min-w-0">
                                <p className="text-[13px] text-white font-medium truncate">{pendingFile ? pendingFile.name : (profile?.display_name || 'Your avatar')}</p>
                                <p className="text-[12px] text-zinc-500">{pendingFile ? `${(pendingFile.size / 1024).toFixed(0)} KB — ready to save` : 'JPG, PNG or GIF · max 5 MB'}</p>
                                <button onClick={() => fileInputRef.current?.click()} className="self-start flex items-center gap-1.5 px-3 py-1.5 rounded-[8px] text-[12px] font-medium transition-all hover:brightness-125" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.7)' }}>
