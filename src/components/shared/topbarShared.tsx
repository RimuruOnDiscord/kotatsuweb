...
interface TopbarSearchResultsProps {
  ...
  onOpenManga: (slug: string) => void; // CHANGED to expect a string (the slug) instead of a number
  onSubmitSearch: () => void;
}
      ...
      {searchResults.length > 0 ? (
        searchResults.map((manga) => (
          <button
            key={manga.mal_id}
            type="button"
            onClick={() => onOpenManga(createSlug(manga.title))} // CHANGED to pass the slugified title instead of the ID
