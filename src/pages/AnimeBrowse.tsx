      ...
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="mb-2 flex items-center gap-2">
            ...
            {getAnimeScore(anime) ? (
              <span className="flex items-center gap-1 rounded-full border border-white/[0.07] bg-white/[0.03] px-2.5 py-0.5 text-[10px] font-semibold text-white/90">
      ...
      <div className="mt-auto rounded-[1.15rem] bg-[var(--app-card)] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
        <div className="grid grid-cols-[1.2fr_.8fr_1fr] gap-3">
          <div className="min-w-0 border-r border-white/[0.05] pr-3">
            ...
            <span className="mt-1 block text-sm font-semibold capitalize text-[var(--app-accent)]">{getAnimeStatusLabel(anime.status)}</span>
          </div>
          <div className="min-w-0 border-r border-white/[0.05] pr-3">
            <span className="block text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Started</span>
            <span className="mt-1 block text-sm font-medium text-zinc-200">{anime.startDate?.year || anime.seasonYear || 'N/A'}</span>
          </div>
          <div className="min-w-0">
            <span className="block text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Episodes</span>
              ...
              {visiblePages.map((page, index) => {
                ...
                return (
                  <React.Fragment key={page}>
                    {shouldShowGap ? <span className="px-2 text-sm font-black text-zinc-600">...</span> : null}
                    <button
                      type="button"
                      onClick={() => commitBrowseParams({ page })}
                      onMouseDown={handleRippleMouseDown}
                      className={`ripple-button inline-flex h-11 min-w-[44px] items-center justify-center rounded-full border px-4 text-[10px] font-black uppercase tracking-[0.18em] transition-colors ${currentPage === page ? 'border-[var(--app-border)] text-[#04110d]' : 'border-white/[0.08] bg-white/[0.03] text-zinc-300 hover:bg-white/[0.06] hover:text-white'}`}
