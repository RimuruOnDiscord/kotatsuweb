...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (playerRef.current) {
        playerRef.current.pause(); // Stop playing old video behind the loading screen
      }
      setStreamLoading(true); 
      // DO NOT clear setStreamData(null) here so Vidstack seamlessly swaps source strings!
      ...
      try {
        ...
        if (mounted) {
          ...
          setStreamData(null); // only clear on error
        }
      } finally {
  ...
  useEffect(() => {
    let mounted = true;
    // DO NOT clear proxifiedStreamUrl here so it stays seamlessly mounted
  ...
  const finalStreamUrl = useMemo(() => {
    ...
    // By keeping the proxified URL and active stream URL, the player avoids getting 'undefined'
    // which prevents the browser from exiting fullscreen due to DOM destruction.
  ...
  const handleEpisodeClick = useCallback((targetId: string) => {
    ...
    // Preserve fullscreen state before navigating to next episode
  ...
  // Restore fullscreen after episode transition when video is ready
  useEffect(() => {
    if (isVideoReady && !streamLoading && pendingFullscreenRestoreRef.current) {
      pendingFullscreenRestoreRef.current = false;
      // Only restore if user hasn't manually re-entered fullscreen since navigation
      if (!document.fullscreenElement && videoContainerRef.current) {
    ...
    <div className="aw-root aw-noise min-h-screen flex flex-col relative">
      ...
      <div className="aw-layout">
        <main className="aw-main">
          ...
          >
            {/* 1. Loading Overlay (Stays on top without unmounting the player) */}
            ...
            {/* 3. Player (Always mounted unless it's a hard iframe embed) */}
