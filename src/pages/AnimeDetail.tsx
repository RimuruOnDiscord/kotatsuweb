...
.aw-btn-primary {
  ...
  .aw-btn-primary:hover {
    ...
    z-index: 10; /* Added: pulls button to the front */
  }
  ...
  .aw-collapse-container {
    ...
    /* Increased negative margins provide ample horizontal room for scale/shadows to bleed without clipping */
    margin: 0 -20px;
    padding: 0 20px;
  }
  ...
  .aw-collapse-content {
    /* Increased left/right padding gently indents content away from strict hidden edges */
...
useEffect(() => {
    const syncBookmarkState = async () => {
      ...
      if (user) {
        try {
          const { data: dbData } = await supabase.from('anime_bookmarks').select('mal_id, title').eq('user_id', user.id);
          if (dbData) {
...
const handleBookmarkToggle = useCallback(async () => {
    ...
    try {
      // Prefer exact matched ID to prevent orphans. On creation, enforce idMal fallback to id.
