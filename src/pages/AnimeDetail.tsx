...
import {
  ChevronLeft, ChevronRight, Star, Loader2, Bookmark, Languages,
...
const AnimeDetail: React.FC = () => {
  ...
  // Generate Navigation Pill Tabs (Recursive-like Iterative Deep Search)
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        setData(info);
        setBookmarked(false); // Reset bookmark state immediately on new anime load
    ...
    // Force one-time reload on initial visit to an anime page to ensure full state sync
    const reloadKey = `mv_reload_${urlSlug}`;
  ...
  useEffect(() => {
    ...
    const syncBookmarkState = async () => {
      // Don't sync if we're currently in the middle of a manual toggle
      if (isSyncingBookmark.current) return;

      const targetId = String(data.idMal || data.id);
      if (user) {
        const { data: dbData } = await supabase.from('anime_bookmarks').select('mal_id').eq('user_id', user.id).eq('mal_id', targetId);
        // Only update if not currently syncing to avoid overwriting optimistic state
        if (!isSyncingBookmark.current) {
          setBookmarked(dbData && dbData.length > 0 ? true : false);
        }
      } else {
        setBookmarked(isBookmarked(Number(targetId), readBookmarks()));
      }
    };
    syncBookmarkState();
      ...
      if (user) {
        if (previousState) {
          ...
          if (error) {
            ...
            setBookmarked(previousState); // Rollback
          }
        } else {
          // Add to DB
        ...
        const result = toggleBookmark({ malId: Number(targetId), title, cover: coverUrl, type: data.format || 'Anime', status: data.status, score: data.averageScore, author: data.studios?.[0]?.name });
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1460px] px-4 pt-8">
        ...
        <div className="flex flex-col md:flex-row gap-8 lg:gap-12 mb-12">
          ...
          <div className="flex-1 flex flex-col justify-end pb-2">
            ...
            <div className="aw-synopsis-container">
              <div
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-12">
          ...
          <div className="space-y-10 min-w-0">
            <div className="flex flex-col min-w-0">
              ...
              {navTabs.length > 1 && (
                <div className="flex overflow-x-auto gap-3 mb-10 pb-2 pt-2 aw-scrollbar">
                  {navTabs.map((tab) => (
                    <button
                      key={tab.id}
                      ...
                      onMouseEnter={(e) => { if (!tab.active) { e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 90%)'; e.currentTarget.style.color = 'white'; e.currentTarget.style.borderColor = 'var(--aw-accent-dim)'; } }}
                      onMouseLeave={(e) => { if (!tab.active) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--aw-muted)'; e.currentTarget.style.borderColor = 'var(--aw-border)'; } }}
                    >
                      {tab.displayLabel}
                    </button>
                  ))}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex flex-wrap items-center gap-2">
                    {providerNames.map((providerName) => (
                      <button
                        ...
                        onMouseEnter={(e) => { if (provider !== providerName) { e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 90%)'; e.currentTarget.style.color = 'white'; e.currentTarget.style.borderColor = 'var(--aw-accent-dim)'; } }}
                        onMouseLeave={(e) => { if (provider !== providerName) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--aw-muted)'; e.currentTarget.style.borderColor = 'var(--aw-border)'; } }}
                      >
                        {providerName}
                      </button>
                    ))}
                    ...
                    {(['sub', 'dub'] as const).map((audioMode) => (
                      <button
                        ...
                        onMouseEnter={(e) => { if (category !== audioMode && (episodesData[provider]?.episodes?.[audioMode]?.length || 0) !== 0) { e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 90%)'; e.currentTarget.style.color = 'white'; e.currentTarget.style.borderColor = 'var(--aw-accent-dim)'; } }}
                        onMouseLeave={(e) => { if (category !== audioMode && (episodesData[provider]?.episodes?.[audioMode]?.length || 0) !== 0) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--aw-muted)'; e.currentTarget.style.borderColor = 'var(--aw-border)'; } }}
                      >
                        {audioMode}
                      </button>
                    ))}
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                      ...
                      onMouseEnter={(e) => { e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 90%)'; e.currentTarget.style.color = 'white'; e.currentTarget.style.borderColor = 'var(--aw-accent-dim)'; }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--aw-muted)'; e.currentTarget.style.borderColor = 'var(--aw-border)'; }}
                    >
                    ...
                    visibleEpisodes.length > 0 ? (
                      visibleEpisodes.map((episode) => (
                        <div
                          ...
                          className="group flex items-start gap-4 p-4 rounded-[14px] transition-all duration-300 cursor-pointer border mb-2 mx-1"
                          style={{ borderColor: 'transparent', background: 'transparent' }}
                          onMouseEnter={(e) => { e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 90%)'; e.currentTarget.style.borderColor = 'var(--aw-accent)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
          ...
          <div className="hidden lg:flex flex-col justify-start pb-2 w-[320px] flex-shrink-0">
            ...
            <div className="space-y-4">
              <div className={`aw-collapse-container ${!topStatsCollapsed ? 'expanded' : ''}`}>
                <div className="aw-collapse-content grid grid-cols-2 gap-3" style={{ paddingTop: 0 }}>
                  <div className="p-4 rounded-[16px] shadow-lg flex flex-col gap-1 transition-all duration-300" style={{ background: 'var(--aw-s1)', border: '1px solid var(--aw-border)' }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--aw-accent)'; e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 92%)'; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--aw-border)'; e.currentTarget.style.background = 'var(--aw-s1)'; }}>
                  ...
                  <div className="col-span-2 p-4 rounded-[16px] shadow-lg flex flex-col gap-1 transition-all duration-300" style={{ background: 'var(--aw-s1)', border: '1px solid var(--aw-border)' }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--aw-accent)'; e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 92%)'; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--aw-border)'; e.currentTarget.style.background = 'var(--aw-s1)'; }}>
                  ...
                  <div className="col-span-2 p-4 rounded-[16px] shadow-lg flex flex-col transition-all duration-300" style={{ background: 'var(--aw-s1)', border: '1px solid var(--aw-border)' }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--aw-accent)'; e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 92%)'; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--aw-border)'; e.currentTarget.style.background = 'var(--aw-s1)'; }}>
                        ...
                        <div className="space-y-4">
                          {reviews.slice(0, 5).map((review) => (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="block p-4 rounded-[16px] border transition-all duration-300 group/rev"
                              style={{ background: 'var(--aw-s1)', borderColor: 'var(--aw-border)' }}
                              onMouseEnter={(e) => { e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 94%)'; e.currentTarget.style.borderColor = 'var(--aw-accent)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                              onMouseLeave={(e) => { e.currentTarget.style.background = 'var(--aw-s1)'; e.currentTarget.style.borderColor = 'var(--aw-border)'; e.currentTarget.style.transform = 'none'; }}
                            >
                              <div className="flex items-center gap-2 mb-2">
                                <img src={review.user.avatar.medium} alt={review.user.name} className="w-6 h-6 rounded-full" />
                    ...
                    <div className="aw-collapse-content space-y-2">
                      {streamingLinks.map((link: any, idx: number) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-3 p-3 rounded-[12px] border transition-all duration-300 group/link"
                          style={{ background: 'var(--aw-s1)', borderColor: 'var(--aw-border)' }}
                          onMouseEnter={(e) => { 
                            e.currentTarget.style.transform = 'translateX(6px)';
                            e.currentTarget.style.background = 'color-mix(in srgb, var(--aw-accent), transparent 94%)';
                            e.currentTarget.style.borderColor = 'var(--aw-accent)';
                          }}
                          onMouseLeave={(e) => { 
                            e.currentTarget.style.transform = 'none';
                            e.currentTarget.style.background = 'var(--aw-s1)';
                            e.currentTarget.style.borderColor = 'var(--aw-border)';
                          }}
                        >
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white/5 group-hover/link:bg-[var(--aw-accent-dim)] transition-colors">
                            {link.site.toLowerCase().includes('youtube') ? (
                              <Youtube size={16} className="text-white group-hover/link:text-[var(--aw-accent)]" />
                            ) : (
                              <ExternalLink size={14} className="text-white group-hover/link:text-[var(--aw-accent)]" />
                            )}
                          </div>
