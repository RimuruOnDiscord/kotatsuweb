...
const fetchFranchise = async (searchTitle: string) => {
  if (!searchTitle) return [];
  
  const cacheKey = `ani_franchise_${searchTitle}`;
  const cached = sessionStorage.getItem(cacheKey);
  if (cached) return JSON.parse(cached);

  const query = `
    query SearchFranchise($search: String) {
  ...
  const navTabs = useMemo(() => {
    ...
    const addTab = (item: any, active = false) => {
      ...
      if (!seenIds.has(item.id)) {
        seenIds.add(item.id);
        tabs.push({ ...item, active });
      }
    };

    // The Current Season
    addTab({
      id: data.anilist_id || data.mal_id,
      title: data.title_english || data.title_romaji || data.title,
      format: data.type,
      slug: resolvedSlug
    }, true);
    ...
    // Deep Franchise matches
    data.franchise?.forEach(f => addTab(f));

    // Sort ascending by ID (Chronological)
    tabs.sort((a, b) => a.id - b.id);

    // Extract dynamic labels to prevent generic "Season X" generation
    const baseTitle = getBaseTitle(data.title_english || data.title_romaji || data.title || '');

    return tabs.map((tab, index) => ({
      ...tab,
      displayLabel: generateTabLabel(tab.title, baseTitle, index)
    }));
  }, [data, resolvedSlug]);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        const baseTitle = getBaseTitle(animeData.title_english || animeData.title_romaji || animeData.title);
        const [episodesPayload, franchiseMediaRaw] = await Promise.all([
          fetchAnimeEpisodes(aniMedia.id),
          fetchFranchise(baseTitle).catch(() => []) 
        ]);

        const lowerBase = baseTitle.toLowerCase();
        const excludedFormats = ['MOVIE', 'OVA', 'SPECIAL', 'MUSIC'];
        
        const franchiseMedia = franchiseMediaRaw.filter((m: any) => {
          if (excludedFormats.includes(m.format)) return false;
          const t1 = (m.title.english || '').toLowerCase();
          const t2 = (m.title.romaji || '').toLowerCase();
          const t3 = (m.title.userPreferred || '').toLowerCase();
          
          // Strict filtering to prevent finding unrelated anime if the base title is too short (e.g. "K")
          if (lowerBase.length <= 3) {
             const exactMatch = (t: string) => t === lowerBase || t.startsWith(`${lowerBase} `) || t.startsWith(`${lowerBase}:`) || t.startsWith(`${lowerBase}-`);
             return exactMatch(t1) || exactMatch(t2) || exactMatch(t3);
          }
          
          return t1.includes(lowerBase) || t2.includes(lowerBase) || t3.includes(lowerBase);
        });

        animeData.franchise = franchiseMedia.map((m: any) => ({
          id: m.id,
          title: m.title.english || m.title.romaji || m.title.userPreferred || '?',
          slug: createSlug(m.title.english || m.title.romaji || m.title.userPreferred || ''),
          format: m.format,
                        ...
                        <div className="flex overflow-x-auto gap-3 mb-10 pb-2 scrollbar-thin scrollbar-thumb-white/10 hover:scrollbar-thumb-white/20 scrollbar-thumb-rounded-full">
                            {navTabs.map((tab) => (
                                <button
                                    ...
                                    className={`px-5 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all outline-none border ${
                                        tab.active 
