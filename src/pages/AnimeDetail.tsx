...
const fetchJikanFallback = async (slug: string): Promise<AnimeData> => {
  const isNumeric = /^\d+$/.test(slug);
  let url = '';
  
  if (isNumeric) {
    url = `https://api.jikan.moe/v4/anime/${slug}/full`;
  } else {
    const query = encodeURIComponent(slug.replace(/-/g, ' '));
    url = `https://api.jikan.moe/v4/anime?q=${query}&order_by=popularity&sort=asc&limit=1`;
  }
  ...
  if (!rawData) {
    throw new Error('No anime found on Jikan fallback');
  }
  ...
  return {
    ...
    title_romaji: rawData.title_japanese, // Approximate fallback
    title_japanese: rawData.title_japanese,
    synopsis: rawData.synopsis,
    episodes: rawData.episodes,
    duration: rawData.duration ? parseInt(rawData.duration) : undefined, // Parse "24 min" -> 24
    score: rawData.score,
    ...
    seasonYear: rawData.year,
    published: rawData.aired ? { string: rawData.aired.string } : undefined,
    studios: rawData.studios?.map((s: any) => ({ name: s.name })),
    genres: rawData.genres?.map((g: any) => ({ mal_id: g.mal_id, name: g.name })),
    images: {
      jpg: {
        image_url: rawData.images?.jpg?.image_url || '',
        large_image_url: rawData.images?.jpg?.large_image_url || '',
      }
    },
    bannerImage: undefined, // Unavailable in Jikan
    externalLinks: rawData.external?.map((e: any) => ({ site: e.name, url: e.url })),
    relations: rawData.relations?.flatMap((r: any) => 
        r.entry.filter((e: any) => e.type === 'anime').map((e: any) => ({
            id: e.mal_id,
            title: e.name,
            slug: createSlug(e.name),
            format: e.type,
...
const navTabs = useMemo(() => {
  ...
  const baseTitle = getBaseTitle(
    data.title_english || data.title_romaji || data.title || ''
  );
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      try {
        ...
        animeData = await fetchJikanFallback(urlSlug);
      }
