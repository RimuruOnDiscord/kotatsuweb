...
// Original Provider Ranking exactly as requested
...
const AnimeWatch: React.FC = () => {
  ...
  const currentIndex = useMemo(() => {
    ...
    if (idx === -1) {
      ...
      if (epNumMatch) {
        idx = providerEpisodes.findIndex(ep => String(ep.number) === epNumMatch[0]);
      }
    }
    return idx;
  ...
  useEffect(() => {
    ...
    if (streamData && episodeId) {
      playingEpisodeRef.current = episodeId;
    }
  ...
  const handleEpisodeClick = useCallback((targetId: string) => {
    ...
    if (activePrefix) {
      ...
      if (targetNumMatch) {
        finalTargetId = `${activePrefix}-${targetNumMatch[0]}`;
      }
    }

    navigate(getEpisodeHref(urlSlug, currentProvider, currentCategory, finalTargetId));
  }, [urlSlug, currentProvider, currentCategory, navigate, forceSaveProgress, episodeId]);
  ...
  const handleCategorySwitch = useCallback((newCat: 'sub' | 'dub') => {
    ...
    navigate(getEpisodeHref(urlSlug, currentProvider, newCat, finalTargetId));
  ...
  const handleVideoEnd = useCallback(() => {
    if (autoPlay && hasNext && !customStreamUrl && providerEpisodes[currentIndex + 1]) {
        ...
        <main className="aw-main">
          ...
          <div style={{ padding: '28px 28px', background: 'var(--aw-s1)', borderRadius: 16, border: '1px solid var(--aw-border)', display: 'flex', flexDirection: 'column', gap: 0 }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', background: 'var(--aw-s1)', border: '1px solid var(--aw-border)', borderRadius: 100, padding: 4, gap: 2 }}>
                  ...
                  <div style={{ flex: '0 0 auto' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                      {rankedProviders.map((p) => {
                        ...
                        return (
                          <button key={p} onClick={() => { setCustomStreamUrl(''); handleProviderSwitch(p); }} className="aw-action-hover" style={{ position: 'relative', padding: '8px 16px', borderRadius: 10, border: isActive ? '1px solid var(--aw-accent)' : '1px solid rgba(255,255,255,0.1)', background: isActive ? 'var(--aw-accent-dim)' : 'rgba(255,255,255,0.03)', color: isActive ? 'var(--aw-accent)' : 'rgba(255,255,255,0.6)', fontSize: 12, fontFamily: 'var(--aw-font-display)', fontWeight: 700, cursor: 'pointer', textTransform: 'lowercase', display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <div style={{ flex: '1 1 auto' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                      {['animepahe', 'animekai', 'animedunya', 'anikoto'].map((prefix) => {
                        const isActive = episodeId?.toLowerCase().startsWith(prefix) && !customStreamUrl;
                        return (
                          <button
                            key={prefix}
                            onClick={() => {
                              ...
                              if (urlSlug) {
                                navigate(getEpisodeHref(urlSlug, currentProvider, currentCategory, newEpisodeId));
                              }
                            }}
                            className="aw-action-hover"
                            style={{ position: 'relative', padding: '8px 16px', borderRadius: 10, border: isActive ? '1px solid var(--aw-accent)' : '1px solid rgba(255,255,255,0.1)', background: isActive ? 'var(--aw-accent-dim)' : 'rgba(255,255,255,0.03)', color: isActive ? 'var(--aw-accent)' : 'rgba(255,255,255,0.6)', fontSize: 12, fontFamily: 'var(--aw-font-display)', fontWeight: 700, cursor: 'pointer', textTransform: 'lowercase', display: 'flex', alignItems: 'center', gap: 6 }}
                          >
                            {isActive && <div className="aw-shimmer-wrapper"><div className="aw-btn-shimmer" /></div>}
          ...
          <div className="aw-scroll" style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
            {loadingEpisodes ? (
              ...
              visibleEpisodes.map((ep, idx) => {
                const isActive = (extractSlug(ep.id) === episodeId || String(ep.number) === episodeId?.match(/\d+$/)?.[0]) && !customStreamUrl;
                return (
