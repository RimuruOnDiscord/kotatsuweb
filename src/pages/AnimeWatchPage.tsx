...
const AnimeWatch: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isHls && Hls.isSupported()) {
      ...
      const finalUrl = activeStream.referer
        ? `/hls-proxy?url=${encodeURIComponent(activeStream.url)}&referer=${encodeURIComponent(activeStream.referer)}`
        : activeStream.url;
        ...
        if (d.fatal) {
          if (d.type === Hls.ErrorTypes.NETWORK_ERROR) {
            if (retryCount < 3) {
              ...
              hls.destroy();
              setStreamError('Stream network blocked or unavailable. Try another server.');
            }
          } else if (d.type === Hls.ErrorTypes.MEDIA_ERROR) {
            hls.recoverMediaError();
          } else {
            hls.destroy();
            setStreamError('Fatal stream error.');
          }
        }
      });
      hlsRef.current = hls;
