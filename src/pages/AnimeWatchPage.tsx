...
const DESIGN_STYLES = `
  ...
  /* Redesigned, elegant hover effect for action buttons */
  .aw-action-btn { 
  ...
  .aw-share-modal {
    animation: shareModalIn 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards;
  }
  ...
  .aw-share-backdrop {
    animation: shareBackdropIn 0.2s ease forwards;
  }

  /* Server Tabs */
  .server-tab { transition: all 0.2s cubic-bezier(0.25, 0.46, 0.45, 0.94); position: relative; }
  .server-tab:hover:not([data-active='true']) { background: rgba(255,255,255,0.06) !important; }
  .server-tab[data-active='true'] { box-shadow: 0 0 0 1px var(--aw-accent), 0 4px 16px -8px var(--aw-accent-glow); }

  /* Quality/Source badges */
  .source-badge { transition: all 0.18s ease; }
  .source-badge:hover:not([data-active='true']) { background: rgba(255,255,255,0.06) !important; border-color: rgba(255,255,255,0.15) !important; }
...
const AnimeWatch: React.FC = () => {
  ...
  // ─── Bookmark sync from Supabase ─────────────────────────────────
  useEffect(() => {
    if (!user || !resolvedId) return;
    const targetId = String(resolvedId);
    const check = async () => {
      try {
        const { data } = await supabase.from('anime_bookmarks').select('mal_id').eq('user_id', user.id).eq('mal_id', targetId).maybeSingle();
        setBookmarked(!!data);
      } catch { /* ignore */ }
    };
    check();
  ...
  // Map keyboard shortcuts for frame-by-frame navigation (like YouTube: , and .)
  useEffect(() => {
  ...
  const handleBookmarkToggle = useCallback(async () => {
    ...
    try {
      const targetId = String(resolvedId || urlSlug);
      ...
      if (user) {
        if (previousState) {
          ...
          else showToast('Added to Bookmarks');
        }
      } else {
        const result = toggleBookmark({
          malId: Number(targetId) || 0, title, cover: coverUrl,
  ...
  }, [animeInfo, bookmarked, resolvedId, urlSlug, displayTitle, studioName, user, showToast]);
    ...
    <div className="aw-root aw-noise min-h-screen flex flex-col relative">
      ...
      <div className="aw-layout">
        <main className="aw-main">
                ...
                {(activeStream?.type === 'embed' && activeStream?.url) ? (
                  ...
                  <MediaPlayer
                    ...
                    onCanPlay={() => {
                      ...
                      // Resync with room after new episode loads
                      if (watchTogether.isInRoom) {
                        setTimeout(() => {
                          if (playerRef.current) {
                  ...
                  >
                    ...
                    {/* Let Vidstack handle standard timestamp tooltips dynamically since there's no actual thumbnail spritesheet */}
          ...
          {/* Controls Bar */}
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '14px 20px', background: 'var(--aw-s1)', borderRadius: 12, border: '1px solid var(--aw-border)' }}>
          ...
          {/* Sources Panel — Tabbed provider selector */}
          <div style={{ padding: '24px 24px 16px', background: 'var(--aw-s1)', borderRadius: 24, border: '1px solid var(--aw-border)', display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* Header — stacked */}
            ...
            {currentEpData?.description && (
              <p style={{ margin: 0, fontSize: 13, fontWeight: 300, color: 'var(--aw-muted)', lineHeight: 1.6, maxWidth: 800 }}>
                {currentEpData.description}
              </p>
            )}
            ...
            {sourcesTab === 'player' && streamData && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, paddingLeft: 20 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                      {streamData.streams.map((s, idx) => {
                        ...
                        return (
                          <button key={idx} type="button" onClick={() => setSelectedStreamIndex(idx)} className="source-badge" data-active={isActive} style={{ padding: '8px 16px', borderRadius: 10, border: isActive ? '1px solid var(--aw-accent)' : '1px solid rgba(255,255,255,0.1)', background: isActive ? 'var(--aw-accent-dim)' : 'rgba(255,255,255,0.03)', color: isActive ? 'var(--aw-accent)' : 'rgba(255,255,255,0.6)', fontSize: 12, fontFamily: 'var(--aw-font-display)', fontWeight: 700, textTransform: 'lowercase', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, position: 'relative' }}>
                            {isActive && <div className="aw-shimmer-wrapper"><div className="aw-btn-shimmer" /></div>}
                            {s.quality || 'auto'}
          ...
          {seasons.length > 1 && (
            <div style={{ padding: '24px 28px', background: 'var(--aw-s1)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="aw-scroll" style={{ display: 'flex', gap: 10, overflowX: 'auto', padding: '10px 4px 18px', margin: '-10px -4px 0' }}>
                {seasons.map(s => (
                  <button key={s.id} onClick={() => { if (!s.active) navigate(`/watch/${s.id}/kiwi/sub/animepahe-1`); }} className="aw-action-hover" style={{ flexShrink: 0, position: 'relative', padding: '10px 20px', borderRadius: 10, border: s.active ? '1px solid var(--aw-accent)' : '1px solid rgba(255,255,255,0.1)', background: s.active ? 'var(--aw-accent-dim)' : 'rgba(255,255,255,0.03)', color: s.active ? 'var(--aw-accent)' : 'var(--aw-text)', fontFamily: 'var(--aw-font-display)', fontSize: 12, fontWeight: 600, letterSpacing: '0.05em', cursor: s.active ? 'default' : 'pointer' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 28, padding: '28px 28px', background: 'transparent', borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: bookmarked ? 'color-mix(in srgb, var(--aw-accent) 15%, transparent)' : 'rgba(255,255,255,0.05)',
                    border: bookmarked ? '2px solid var(--aw-accent)' : '1px solid rgba(255,255,255,0.1)',
                    color: bookmarked ? 'var(--aw-accent)' : 'var(--aw-muted)',
                    cursor: 'pointer', transition: 'all 0.2s'
                  }}
                >
                  <Bookmark size={20} fill={bookmarked ? 'currentColor' : 'none'} strokeWidth={bookmarked ? 0 : 2} />
                </button>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20, alignItems: 'center' }}>
                {studioName && (
                  <span className="studio-pill">
                    <Clapperboard size={11} /> {studioName}
                  </span>
                )}
