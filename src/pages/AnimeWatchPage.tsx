...
import {
  ...
  Server, MonitorPlay, Building, ArrowDownUp, Link2, Activity, Database, Terminal
} from 'lucide-react';
...
const DESIGN_STYLES = `
  ...
  @media (min-width: 1180px) {
    ...
    .aw-sidebar { position: sticky; top: 96px; height: calc(100vh - 120px); }
  }
    ...
    const loadAnimeData = async () => {
      ...
      try {
        ...
        const tabs = [{
          ...
          format: info.format,
          active: true,
          displayLabel: generateTabLabel(currentTitle, baseTitle, 0)
        }];

        if (info.relations?.edges) {
          info.relations.edges.forEach((edge: any, idx: number) => {
            const node = edge.node;
            if (node.type === 'ANIME' && allowedRelations.includes(edge.relationType) && !excludedFormats.includes(node.format)) {
              tabs.push({
                id: node.id,
                title: node.title?.english || node.title?.romaji || node.title?.native,
                format: node.format,
                active: false,
                displayLabel: generateTabLabel(node.title?.english || node.title?.romaji || node.title?.native || '', baseTitle, idx + 1)
              });
            }
          });
        }
            ...
            <div style={{ flex: 1, minWidth: 200, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
                {(animeInfo?.genres || ['Anime']).map((g: string) => (
                  <button
                    key={g}
                    onClick={() => navigate(`/anibrowse?genres=${encodeURIComponent(g)}`)}
