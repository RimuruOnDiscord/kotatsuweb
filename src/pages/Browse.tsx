...
const MangaListCard: React.FC<{ manga: BrowseManga; navigate: (path: string) => void }> = ({ manga, navigate }) => (
  <div
    ...
    className="group relative flex h-48 gap-4 overflow-hidden rounded-[1.4rem] border border-[var(--app-border)] bg-[var(--app-surface-1)] p-3 transition-all duration-300 hover:bg-[var(--app-surface-2)] hover:shadow-[0_20px_55px_-30px_rgba(0,0,0,0.85)]"
  >
      ...
      <div className="mt-auto rounded-[1.15rem] bg-[var(--app-card)] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
        <div className="grid grid-cols-[1.2fr_.8fr_1fr] gap-3">
          <div className="min-w-0 border-r border-white/[0.05] pr-3">
            ...
            <span className={`mt-2 block text-[11px] font-black uppercase tracking-[0.1em] ${manga.statusLabel === 'Publishing' ? 'text-[var(--app-accent)]' : 'text-zinc-300'}`}>
              {manga.statusLabel}
            </span>
          </div>
          <div className="min-w-0 border-r border-white/[0.05] pr-3">
