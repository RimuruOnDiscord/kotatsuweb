...
<div className="mt-auto rounded-[1.15rem] bg-[var(--app-card)] p-2 sm:px-4 sm:py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
  {/* Changed to equal cols on mobile, original ratio on sm+ screens. Reduced gap on mobile. */}
  <div className="grid grid-cols-3 gap-1 sm:grid-cols-[1.2fr_.8fr_1fr] sm:gap-3">
    
    {/* Status Section */}
    {/* Reduced pr-3 to pr-1 on mobile */}
    <div className="min-w-0 border-r border-white/[0.05] pr-1 sm:pr-3">
      {/* Added truncate, reduced text size & tracking on mobile */}
    ...
    <div className="min-w-0 border-r border-white/[0.05] px-1 sm:px-0 sm:pr-3">
      ...
      <span className="mt-1 block truncate text-xs sm:text-sm font-medium text-white">
        {manga.year || 'N/A'}
      </span>
    </div>
...
<div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 pt-2">
  <button
    ...
    className="inline-flex h-9 sm:h-11 items-center gap-1.5 sm:gap-2 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 sm:px-4 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.18em] text-zinc-300 transition-all duration-300 hover:border-white/[0.15] hover:bg-[var(--app-surface-2)] hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
  >
    ...
    return (
      <React.Fragment key={page}>
        ...
        <button
          ...
          className={`inline-flex h-9 sm:h-11 min-w-[36px] sm:min-w-[44px] items-center justify-center rounded-full border px-3 sm:px-4 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.18em] transition-all duration-300 ${
            currentPage === page
