      ...
      <div className="mt-auto rounded-[1.15rem] bg-[var(--app-card)] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
        <div className="grid grid-cols-[1.2fr_.8fr_1fr] gap-3">
          <div className="min-w-0 border-r border-white/[0.05] pr-3">
            ...
            <span className={`mt-1 block text-sm font-semibold capitalize ${manga.statusLabel === 'Publishing' ? 'text-[var(--app-accent)]' : 'text-zinc-200'}`}>
              {manga.statusLabel}
            </span>
          </div>
          <div className="min-w-0 border-r border-white/[0.05] pr-3">
  ...
  >
    <div className="relative aspect-[2/3] w-full overflow-hidden bg-[var(--app-card)]">
      ...
      <div className="absolute left-2 top-2 flex flex-col gap-1">
        <span className="w-fit rounded-full border border-white/[0.07] bg-black/70 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white backdrop-blur-md">
          {manga.typeLabel}
        </span>
      </div>

      {manga.score ? (
        <div className="absolute right-2 top-2 flex items-center gap-1 rounded-full border border-white/[0.07] bg-black/70 px-2 py-1 text-[10px] font-semibold text-white backdrop-blur-md">
          <Star size={10} className="fill-amber-400 text-amber-400" />
          {manga.score.toFixed(1)}
        </div>
      ) : null}
