      ...
      <aside className={`absolute right-0 top-0 z-[80] h-full w-full max-w-[390px] border-l border-white/5 bg-[#0b0c0f]/96 shadow-[0_40px_120px_rgba(0,0,0,0.9)] backdrop-blur-2xl transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex h-full flex-col p-5 sm:p-6">
          ...
          <div className="space-y-5 overflow-y-auto thin-scroll pr-1">
            ...
            <section className="rounded-[28px] border border-white/10 bg-white/[0.03] p-5">
              <div className="mb-3 text-[9px] font-black uppercase tracking-[0.26em] text-gray-500">Export</div>
              <button
                type="button"
                onClick={handleExportHtml}
                onMouseDown={handleRippleMouseDown}
                disabled={isExporting || pages.length === 0}
                className="ripple-button flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-gray-300 transition-all hover:border-emerald-500/30 hover:bg-emerald-500/10 hover:text-emerald-400 disabled:cursor-not-allowed disabled:opacity-30"
              >
