  ...
  useEffect(() => {
    ...
    s.innerHTML = `
      ...
      :root {
        ...
        --rd-font-body: 'Onest', sans-serif;
      }

      .rd-root { font-family: var(--rd-font-body); }
      ...
      .thin-scroll::-webkit-scrollbar-thumb:hover { background: var(--app-accent); }
      ...
      /* Chapter item hover */
      .ch-item { transition: background 0.18s, border-color 0.18s; }
...
<body>
  ...
  return (
    <div className="rd-root rd-noise relative flex h-screen w-full overflow-hidden bg-[var(--app-bg)] text-white">
      {/* Ambient glow */}
      ...
      <main className="relative z-10 flex min-w-0 flex-1 flex-col overflow-hidden" style={{ background: 'var(--app-bg)' }}>
        <header className={`border-b border-[var(--app-border)] backdrop-blur-2xl transition-all duration-300 ${focusMode ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`} style={{ background: 'rgba(var(--app-bg-rgb, 10,10,10), 0.85)' }}>
          <div className="flex items-center justify-between gap-4 px-4 py-3 sm:px-6">
            <div className="flex min-w-0 items-center gap-3 sm:gap-5">
              <button
                onClick={() => navigate(-1)}
                onMouseDown={handleRippleMouseDown}
                className="ripple-button flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[var(--app-border)] bg-white/[0.03] text-zinc-400 transition-all hover:border-[var(--app-accent)] hover:text-[var(--app-accent)] hover:bg-white/[0.06]"
              >
                <ChevronLeft size={20} />
              </button>
              <button 
              ...
              >
                <Logo />
                <span style={{ fontFamily: 'var(--rd-font-display)' }} className="text-lg font-bold tracking-tight text-white transition-colors group-hover:text-[var(--app-accent)]">
                  kotatsuweb
                </span>
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setFocusMode((current) => !current)} onMouseDown={handleRippleMouseDown} className="ripple-button flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--app-border)] bg-white/[0.03] text-zinc-400 transition-all hover:border-[var(--app-accent)] hover:text-[var(--app-accent)] hover:bg-white/[0.06]">
                {focusMode ? <Eye size={16} /> : <EyeOff size={16} />}
              </button>
              <button onClick={() => setSidebarOpen((current) => !current)} onMouseDown={handleRippleMouseDown} className={`ripple-button flex h-10 w-10 items-center justify-center rounded-xl border transition-all ${sidebarOpen ? 'border-[var(--app-accent)] bg-[var(--app-accent)] text-black shadow-[0_0_16px_var(--app-accent-muted)]' : 'border-[var(--app-border)] bg-white/[0.03] text-[var(--app-accent)] hover:border-[var(--app-accent)] hover:bg-white/[0.06]'}`}>
                <Layout size={16} />
              </button>
            </div>
          </div>
        ...
        <footer className={`h-1 transition-opacity duration-300 ${focusMode ? 'opacity-0' : 'opacity-100'}`} style={{ background: 'var(--app-border)' }}>
          <div 
            className="h-full transition-all duration-500 ease-out" 
            style={{ 
              width: `${progressPercent}%`, 
              backgroundColor: 'var(--app-accent)',
              boxShadow: '0 0 20px 2px var(--app-accent), 0 0 40px 4px var(--app-accent-muted)',
      ...
      <aside className={`absolute right-0 top-0 z-[80] h-full w-full max-w-[390px] border-l border-[var(--app-border)] shadow-[0_0_80px_rgba(0,0,0,0.5)] backdrop-blur-2xl transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`} style={{ background: 'rgba(var(--app-bg-rgb, 10,10,10), 0.92)' }}>
        <div className="flex h-full flex-col p-5 sm:p-6">
          <div className="mb-6 flex items-center justify-between">
            <div>
              <div className="rd-label">Reader Console</div>
              <div style={{ fontFamily: 'var(--rd-font-display)' }} className="mt-1 text-sm font-bold tracking-wide text-white">Control Surface</div>
            </div>
            <button onClick={() => setSidebarOpen(false)} onMouseDown={handleRippleMouseDown} className="ripple-button flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--app-border)] bg-white/[0.03] text-zinc-400 transition-all hover:text-white hover:bg-white/[0.06]">
              <X size={16} />
            </button>
          </div>
          ...
          <div className="space-y-5 overflow-y-auto thin-scroll pr-1">
            ...
            <section className="rounded-2xl border border-[var(--app-border)] bg-white/[0.02] p-5">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <div className="rd-label">Chapter Browser</div>
                  <div style={{ fontFamily: 'var(--rd-font-display)' }} className="mt-1 text-sm font-bold tracking-wide text-white">Fast Navigation</div>
                </div>
                <button onClick={() => setChapterBrowserOpen((current) => !current)} onMouseDown={handleRippleMouseDown} className="ripple-button flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--app-border)] bg-white/[0.03] text-zinc-400 transition-transform hover:text-white hover:border-zinc-500/50 hover:bg-white/[0.06]">
                  <ChevronDown size={16} className={`transition-transform duration-300 ${chapterBrowserOpen ? 'rotate-180' : ''}`} />
                </button>
              </div>
            ...
            <section className="rounded-2xl border border-[var(--app-border)] bg-white/[0.02] p-5">
              <div className="mb-3 rd-label">Reader Mode</div>
            ...
            <section className="rounded-2xl border border-[var(--app-border)] bg-white/[0.02] p-5">
              <div className="mb-3 rd-label">Export</div>
              <button
                type="button"
            ...
            <section className="rounded-2xl border border-[var(--app-border)] bg-white/[0.02] p-5">
              <div className="mb-3 rd-label">Shortcuts</div>
